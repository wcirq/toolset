@echo off
setlocal
set "PROJECT=%~dp0CoolCatCloseGuard.vcxproj"
...
:vswhere_found
set "RESULT_FILE=%TEMP%\coolcat_msbuild_%RANDOM%_%RANDOM%.txt"
...
if not "%errorlevel%"=="0" (
  echo.
  echo If LNK1104 names CoolCatCloseGuard64.dll, the existing DLL is locked by a running CoolCat/Python process or security software.
)
exit /b %errorlevel%
